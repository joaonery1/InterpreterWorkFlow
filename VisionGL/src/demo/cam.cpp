...
void display(void)
{
 ...
 // vglVerticalFlip(img[0],img[1]); //Flip image vertically i.e. top becomes bottom

  //vglZoom(img[0],img[1],1.8);     //Zoom

  //vglJulia(img[5]);               //direta baixo

  vglContrast(img[0],img[1],1.5); //direita cima

  //vglInOut(img[0],img[1]);

  //vglBaricenterInit(img[0],img[1]); //erro no input

  //vgl1to3Channels(img[0],img[2]); //mesmo erro equivalente ao Gray ~ Convert grayscale image to RGB
  //vgl3dNot(img[0],img[1]);        //erro do vec;

  //vglAbsDiff(img[4], img[5], img[7]); //Absolute difference between two images.
  vglBlurSq3(img[0],img[1]);
